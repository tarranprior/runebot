    ...
    async def prefix(self, ctx: Context, prefix: str):
        if str(load_configuration()['configuration']['prefix']) != prefix:
            with open('config.json', 'r+') as json_file:
                ...
                json_file.truncate()
            embed_prefix = disnake.Embed(title="New Prefix Set", description=f"Prefix updated to `{prefix}` in `config.json`. Please restart the bot to apply the changes.", color=disnake.Colour.og_blurple(), timestamp=ctx.message.created_at)
            embed_prefix.set_footer(text=f"Author: {ctx.message.author.name} (ID: {ctx.message.author.id})")
            return await ctx.send(embed=embed_prefix)
    ...
    async def status(self, ctx: Context, *, status: str):
        if str(load_configuration()['configuration']['activity']) != status:
            with open('config.json', 'r+') as json_file:
                ...
                json.dump(data, json_file, indent=4)
                json_file.truncate()
            embed_prefix = disnake.Embed(title="New Status Set", description=f"Default activity status updated to `{status}` in `config.json`. Please restart the bot to apply the changes.", color=disnake.Colour.og_blurple(), timestamp=ctx.message.created_at)
            embed_prefix.set_footer(text=f"Author: {ctx.message.author.name} (ID: {ctx.message.author.id})")
            return await ctx.send(embed=embed_prefix)
    ...
    async def prefix_slash(self, inter: ApplicationCommandInteraction, prefix: str):
        if str(load_configuration()['configuration']['prefix']) != prefix:
            with open('config.json', 'r+') as json_file:
                ...
                json.dump(data, json_file, indent=4)
                json_file.truncate()
            embed_prefix = disnake.Embed(title="New Prefix Set", description=f"Prefix updated to `{prefix}` in `config.json`. Please restart the bot to apply the changes.", color=disnake.Colour.og_blurple(), timestamp=inter.created_at)
            embed_prefix.set_footer(text=f"Author: {inter.author.name} (ID: {inter.author.id})")
            return await inter.response.send_message(embed=embed_prefix)
        embed = EmbedFactory().create(title='Prefix Error', description=f'Prefix is already set to `{prefix}`. Use `{prefix}botinfo` to fetch all current configuration values.', colour=disnake.Colour.red())
    ...
    async def status_slash(self, inter: ApplicationCommandInteraction, status: str):
        if str(load_configuration()['configuration']['activity']) != status:
            with open('config.json', 'r+') as json_file:
                ...
                json.dump(data, json_file, indent=4)
                json_file.truncate()
            embed_prefix = disnake.Embed(title="New Status Set", description=f"Default activity status updated to `{status}` in `config.json`. Please restart the bot to apply the changes.", color=disnake.Colour.og_blurple(), timestamp=inter.created_at)
            embed_prefix.set_footer(text=f"Author: {inter.author.name} (ID: {inter.author.id})")
            return await inter.response.send_message(embed=embed_prefix)
        embed = EmbedFactory().create(title='Activity Status Error', description=f"Status is already set to `{status}`. Use `{load_configuration()['configuration']['prefix']}botinfo` to fetch all current configuration values.", colour=disnake.Colour.red())
        return await inter.response.send_message(embed=embed)
